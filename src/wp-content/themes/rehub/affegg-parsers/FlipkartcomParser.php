...
class FlipkartcomParser extends ShopParser {
    ...
    public function parseDescription() {
           ...
           //$descr = implode(";\n", $results); 
           $results = $this->xpathScalar(".//div[contains(@class,'rpdSection')]/p[contains(@class,'description')]");              
        return $descr;
    }

    public function parsePrice() {
        $price = preg_replace('/[^0-9]/', '', $this->xpathScalar(".//div[contains(@class,'price-wrap')]//span[contains(@class,'selling-price')]"));
        if (!$price)
            $price = preg_replace('/[^0-9]/', '', $this->xpathScalar(".//div[contains(@class,'pricing')]//span[contains(@class,'selling-price omniture-field')]"));
        return (float) $price;               
    }
