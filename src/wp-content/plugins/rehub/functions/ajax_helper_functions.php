...
function ajax_action_re_filterpost() {  
    ...
    if(!empty($sorttype) && is_array($sorttype)) { //if sorting panel  
        $filtertype = $filtermetakey = $filtertaxkey = $filtertaxtermslug = $filterorder = $filterdate = '';
        ...
        if($filterdate !='all') { //if date sorting
...
function coupon_get_code(){
    ...
    if( !empty( $code ) ){
        ...
        elseif ('product' == get_post_type($codeid)) {
            if (rehub_option('woo_thumb_enable') == '1') {$thumb_enable = '1';}
